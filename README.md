...
The Neon JS SDK is pinned to `0.1.0-beta.22` because this project uses its `BetterAuthReactAdapter`. If Netlify or Vercel previously built a different dependency tree, trigger a clean deploy after this pin so the browser does not keep an old Auth bundle.
